...
// typedef tree<int, null_type, less<int>, rb_tree_tag, tree_order_statistics_node_update> pbds;
 ...
 int main()
 {
    ...
    ll yellowCry,blueCry;
    cin>>yellowCry>>blueCry;
 
    ll x,y,z;
    cin>>x>>y>>z;
 
    ll tyellow=0,tblue=0;
    ...
    if(tyellow<=yellowCry && tblue<=blueCry)
    {
        cout<<0<<"\n";
    }
    else
    {
        if(tyellow<=yellowCry){
            cout<<tblue-blueCry<<endl;
            return 0;
        }
        if(tblue<=blueCry){
            cout<<tyellow-yellowCry<<endl;
            return 0;
        }
        else{
             cout<<(tyellow-yellowCry)+(tblue-blueCry)<<"\n";
             return 0;
         }
