...
int main()
{
    ...
    string s;
    cin>>s;
    for0(i, sz(s)) v.pb(s[i]);
 
    sort(v.begin(), v.end()); //first need to sort the array.
 
    it=unique(v.begin(),v.end()); //discard all the duplicate elements.
 
    v.resize(distance(v.begin() ,it)); //Then resize the vector.
